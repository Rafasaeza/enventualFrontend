...
export default function OpenStreetMap({ coordinates, newCoordinate, position }) {
  ...
  // Si `position` no está disponible, no renderizamos el mapa
  if (!position || position.length !== 2) return null;

  return (
    <div style={{ height: "50vh", width: "100%", position: "relative" }}>
      <MapContainer
        key={position.join(",")} // Usamos la posición para que React fuerce un re-render
        center={position}        // Aquí pasamos la posición para el centro del mapa
        zoom={zoom}
      ...
      >
        ...
        {coordinates && coordinates.length > 0 && coordinates.map((coord, index) => (
